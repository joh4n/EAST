...
def detect(
    score_map, geo_map, timer, score_map_thresh=1e-5, box_thresh=1e-8, nms_thres=0.1
):
    """
...
def predict(model, criterion, epoch):
    ...
    for idx, im_fn in enumerate(im_fn_list):
        ...
        print(
            "EAST <==> TEST <==> idx:{} <==> model  :{:.2f}ms".format(
                idx, timer["net"] * 1000
            )
        )
        ...
        print(
            "EAST <==> TEST <==> idx:{} <==> restore:{:.2f}ms".format(
                idx, timer["restore"] * 1000
            )
        )
        print(
            "EAST <==> TEST <==> idx:{} <==> nms    :{:.2f}ms".format(
                idx, timer["nms"] * 1000
            )
        )
        ...
        if boxes is not None:
            res_file = os.path.join(
                output_dir_txt,
                "res_img_{}.txt".format(
                    os.path.basename(im_fn).split("_")[-1].strip(".jpg")
                ),
            )
