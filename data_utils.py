...
def get_images(root):
    ...
    """
    assert os.path.isdir(root) == True, "get_img get a wrong path:{} to imgs".format(
        root
    )
    ...
    for i in range(len(files)):
        assert (
            os.path.basename(files[i]) == name[i]
        ), "img path cant corresponding to img name"
...
def choose_best_begin_point(pre_result):
    ...
    for coordinate in pre_result:
        ...
        for i in range(4):
            temp_force = (
                calculate_distance(combinate[i][0], dst_coordinate[0])
...
def crop_area(
    im, polys, tags, crop_background=False, max_tries=5000, vis=False, img_name=None
):
    """
...
def point_dist_to_line(p1, p2, p3):
    ...
    try:
        eps = 1e-5
        distance = np.linalg.norm(np.cross(p2 - p1, p1 - p3)) / (
            np.linalg.norm(p2 - p1) + eps
        )
...
def image_label(
    ...
    try:
        ...
        if np.random.rand() < background_ratio:
            ...
            if (
                len(text_polys) == 0
            ):  # for some reason , gt contain no polys, have to return black
                score_map = np.zeros((input_size, input_size), dtype=np.uint8)
                geo_map_channels = 5
                geo_map = np.zeros(
                    (input_size, input_size, geo_map_channels), dtype=np.float32
...
class custom_dset(data.Dataset):
    def __init__(self, img_root, txt_root, vis=False):
        ...
        self._txt_path_list = [
            os.path.join(txt_root, txt_name) for txt_name in os.listdir(txt_root)
        ]
    ...
    def __getitem__(self, index):
        ...
        status = True
        while status:
            img, score_map, geo_map, training_mask = image_label(
                self.txt_root,
                self.img_path_list,
                self.img_name_list,
                self.txt_path_list,
